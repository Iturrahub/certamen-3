...
def registrar_pedido():
    id_pedido = random.randint(100000, 999999)
...
def listar_pedidos():
    ...
    for pedido in pedidos:
        print(f"{pedido['ID']}\t{pedido['Cliente']}\t{pedido['Comuna']}\t\t{pedido['Detalle']['Disp.6lts']}\t{pedido['Detalle']['Disp.10lts']}\t{pedido['Detalle']['Disp.20lts']}")
        ...
        with open(f"hoja_ruta_{sector}.csv", "w") as f:
            ...
            for pedido in pedidos:
                if pedido["Comuna"] == sector:
                    f.write(f"{pedido['ID']},{pedido['Cliente']},{pedido['Comuna']},{sector},{pedido['Detalle']['Disp.6lts']},{pedido['Detalle']['Disp.10lts']},{pedido['Detalle']['Disp.20lts']}\n")
...
def buscar_pedido_por_id():
    ...
    for pedido in pedidos:
        if pedido["ID"] == id_pedido:
            ...
            print(f"Dirección: {pedido['Comuna']}")
